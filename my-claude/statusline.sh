#!/bin/bash
# Generated by cc-statusline v1.5.0 (https://www.npmjs.com/package/@chongdashu/cc-statusline)
# Custom Claude Code statusline - Created: 2026-01-06T20:46:14.901Z
...
to_epoch() {
  ...
  clean="${ts%Z}"; clean="${clean%.*}"; date -u -j -f "%Y-%m-%dT%H:%M:%S%z" "${clean}+0000" +%s 2>/dev/null && return
...
# Session time from native JSONL parsing (no external deps)
SESSION_WINDOW_SECS=$(( 5 * 3600 ))

# Get session_id from Claude's input
session_id=""
if [ "$HAS_JQ" -eq 1 ]; then
...
if [ -n "$session_id" ]; then
  # Build project folder path (encode path by replacing / with -)
  proj_path=""
  if [ "$HAS_JQ" -eq 1 ]; then
    proj_path=$(echo "$input" | jq -r '.workspace.current_dir // .cwd // empty' 2>/dev/null)
  fi
  [ -z "$proj_path" ] && proj_path="$current_dir"

  # Expand ~ and encode path (replace all / with -)
  proj_path="${proj_path/#\~/$HOME}"
  encoded_path=$(echo "$proj_path" | tr '/' '-')

  # Find the JSONL file
  jsonl_file=""
  for base_dir in "$HOME/.claude/projects" "$HOME/.config/claude/projects"; do
    candidate="$base_dir/$encoded_path/$session_id.jsonl"
    if [ -f "$candidate" ]; then
  ...
  if [ -n "$jsonl_file" ] && [ -f "$jsonl_file" ]; then
    # Get first non-null timestamp from JSONL (session start)
    first_ts=""
    if [ "$HAS_JQ" -eq 1 ]; then
      first_ts=$(head -10 "$jsonl_file" | jq -r 'select(.timestamp != null) | .timestamp' 2>/dev/null | head -1)
    fi
    [ -z "$first_ts" ] && first_ts=$(head -10 "$jsonl_file" | grep -o '"timestamp"[[:space:]]*:[[:space:]]*"[0-9T:.Z+-]*"' | head -1 | sed 's/.*"timestamp"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/')

    if [ -n "$first_ts" ]; then
      start_sec=$(to_epoch "$first_ts")
      if [ -n "$start_sec" ] && [ "$start_sec" -gt 0 ] 2>/dev/null; then
        now_sec=$(date +%s)
        # Assume rate limit window starts at session start
        end_sec=$(( start_sec + SESSION_WINDOW_SECS ))
        total=$SESSION_WINDOW_SECS
